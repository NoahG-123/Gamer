...
/**
 * Open a file. Story files return a viewer descriptor. Dressing files open into a
 * plausible synthetic body by type (see lib/synth): text in Notepad, media in the
 * player, images and PDFs in Chrome. Only types with no sensible app (installers,
 * archives, Office documents on a machine without Office) return `openable: false`,
 * which the UI turns into the real Windows "how do you want to open this" dialog.
 * `with` forces a viewer, which is what that dialog's choices use.
 */
export async function POST(req: NextRequest) {
  ...
  if (node.openable) {
    ...
    if (story) {
      if (story.kind === "text" && force !== "chrome") return json({ openable: true, node, viewer: "notepad", text: story.body ?? "", fired });
  ...
  const kind = dressingKind(node.ext);
  if (force === "notepad") return json({ openable: true, node, viewer: "notepad", text: kind === "text" ? "" : junkText(node.path, node.ext, node.size), synthetic: true, fired });
  ...
  switch (kind) {
    case "text": return json({ openable: true, node, viewer: "notepad", text: "", synthetic: true, fired });
