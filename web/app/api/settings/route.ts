...
import { getSettings, setSettings, networkProfile, SettingsState } from "@/lib/settings";
import { recordEvent } from "@/lib/state";
import { runDueReplies } from "@/lib/messaging";
import { json } from "@/lib/http";
export const dynamic = "force-dynamic";

export async function GET() { return json({ settings: getSettings(), network: networkProfile() }); }
...
export async function POST(req: NextRequest) {
  ...
  return json({ settings, network: networkProfile() });
}
